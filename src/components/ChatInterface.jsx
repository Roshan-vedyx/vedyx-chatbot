...
const ChatInterface = ({ user, messages = [], input, setInput, sendMessage, loading, chatHistory = [], loadChat, startNewChat }) => {
  ...
  return (
    <Flex height="100vh" bg={bgColor} flexDirection={{ base: "column", md: "row" }}>
      <Box w={sidebarCollapsed ? "60px" : "250px"} minW={sidebarCollapsed ? "60px" : "250px"} transition="width 0.3s" bg={sidebarBg} boxShadow="md" p={sidebarCollapsed ? 2 : 4} overflowY="auto" display="flex" flexDirection="column">
        <IconButton
        ...
        {!sidebarCollapsed && (
          <>
            <Button leftIcon={<FaPlus />} colorScheme="teal" onClick={startNewChat} mb={4}>New Chat</Button>
            <Text fontSize="lg" fontWeight="bold" mb={3}>Chat History</Text>
            ...
            <VStack spacing={2} align="start" mt="auto" pb={2}>
              <Button leftIcon={<FaQuestionCircle />} variant="ghost" width="100%" justifyContent="flex-start">Help</Button>
              <Button leftIcon={<FaCog />} variant="ghost" width="100%" justifyContent="flex-start" onClick={() => navigate("/settings")}>Settings</Button>
            </VStack>
          </>
        )}
      </Box>
      <Box flex="1" maxW="900px" display="flex" flexDirection="column" p={4} mx="auto" width="100%">
          ...
          {messages.length === 0 ? (
            ...
            messages.map((msg, index) => (
              <Flex key={index} justify={msg.sender === "user" ? "flex-end" : "flex-start"} mb={4}>
                <Box p={3} borderRadius="md" bg={msg.sender === "user" ? "blue.100" : "gray.100"} maxW="75%" textAlign="left">
                  <Text>{msg.text}</Text>
                </Box>
              </Flex>
            ))
        ...
        <Box position="sticky" bottom="0" bg="gray.50" p={3} boxShadow="md">
          <Flex align="center">
            <Textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask me anything..." size="lg" resize="none" flex="1" />
            <Button colorScheme="teal" onClick={() => { if (!input.trim()) return; sendMessage(); }} isLoading={loading} loadingText="Thinking..." ml={2}>Send</Button>
          </Flex>
        </Box>
      </Box>
